...
class ElectronicBody extends StatelessWidget {
  const ElectronicBody({super.key});
    ...
    return BlocBuilder<ProductsCubit, ProductsState>(
      builder: (context, state) {
        if (state is ProductsLoading) {
          ...
          if (state.electronicsProducts.isEmpty) {
            return const Center(
              child: Text(
          ...
          return GridView.builder(
            ...
            itemBuilder: (context, index) {
              return ProductCard(product: state.electronicsProducts[index]);
            },
          );
        }
